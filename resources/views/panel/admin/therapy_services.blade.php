<x-panel-layout>
    ...
    <dic class="row justify-content-between">
        <div class="col-lg-7">
            <div class="mb-4">
                <a data-bs-toggle="collapse" href="#collapseAdd" role="button" aria-expanded="false" aria-controls="collapseAdd" class="btn btn-primary with-icon px-4 br-4">
            ...
            <div class="tm-time-item mb-4">
                <form action="{{ route('admin.therapy.add') }}" method="post" class="loading">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea rows="2" name="name" class="form-control" autocomplete="off" placeholder="{{ __('Name') }}" required></textarea>
                    </div>
                    <div class="mb-3">
                        <label>{{ __('Service Description') }}</label>
                        <textarea rows="4" name="description" class="form-control" autocomplete="off" placeholder="{{ __('Description') }}" required></textarea>
                    </div>
    ...
    @foreach($services as $service)
        <div class="tm-time-item flex-between items-center flex-block-mobile">
            ...
            <div class="right flex-end">
                <a class="btn btn-warning fw-medium br-4" data-bs-toggle="collapse" href="#collapseEdit{{$service->id}}" role="button" aria-expanded="false" aria-controls="collapseEdit{{$service->id}}">Edit</a>
                <a href="#modalTherapyDelete{{$service->id}}" class="btn btn-danger fw-medium br-4" data-bs-toggle="modal" data-bs-target="#modalTherapyDelete{{$service->id}}">Delete</a>
            </div>
        </div>
        ...
        <div class="collapse" id="collapseEdit{{$service->id}}">
            <div class="tm-time-item mb-5 mt-0">
                <form action="{{ route('admin.therapy.save') }}" method="post" class="loading">
                    ...
                    <div class="mb-3">
                        <label>{{ __('Service Category') }}</label>
                        <input type="text" name="category" class="form-control" autocomplete="off" placeholder="{{ __('Category') }}" value="{{$service->category}}" required>
                    </div>
                    <div class="mb-3">
                        <label>{{ __('Service Name') }}</label>
                        <textarea rows="2" name="name" class="form-control" autocomplete="off" placeholder="{{ __('Name') }}" required>{{$service->name}}</textarea>
                    </div>
                    <div class="mb-3">
                        <label>{{ __('Service Description') }}</label>
                        <textarea rows="4" name="description" class="form-control" autocomplete="off" placeholder="{{ __('Description') }}" required>{{$service->description}}</textarea>
                    </div>
